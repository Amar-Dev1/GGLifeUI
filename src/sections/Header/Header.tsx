...
const Header: React.FC = () => {
    ...
    return (
        <>
            <div id='header' className='w-[95%] lg:w-[90%] container bg-gray-50 backdrop:blur-sm flex justify-between items-center p-3 mx-auto rounded-full'>
                ...
                <div className='wide-nav hidden lg:flex xl:flex'>
                    ...
                    <div className='about-link px-4 py-2 mx-1 text-black cursor-pointer relative group'>
                        about
                        <img src={arrowDown} alt="" className='w-2.5 absolute right-0 top-1/2 -translate-y-1/2' />
                        <div className='about-links w-56 bg-[rgba(230,230,230,0.5)] backdrop-blur-sm rounded-2xl flex flex-col py-3 px-5 shadow absolute left-1/2 top-12 -translate-x-1/2 scale-0 group-hover:scale-100 transition-all duration-300'>
                            <a href="#showcase" className='text-black my-1 hover:translate-x-4 transition-transform duration-300'>Why GGLife ?</a><hr className='opacity-25' />
                            <a href="#about" className='text-black my-1 hover:translate-x-4 transition-transform duration-300'>About GGLife</a>
                        </div>
                    </div>
                    <a className='px-4 py-2 mx-1 text-black' href="#contact">Contact us</a>
                </div>
                <div className='sub-wide-nav hidden lg:flex xl:flex'>
                    <a href="/signup" className='px-4 mx-2 py-3 text-black'>Get Started</a>
                    <GGlink title='Login' className='bg-black text-white px-4' takeTo={'/login'} />
                </div>
            ...
            <div className={`narrow-nav text-center flex flex-col justify-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[rgba(230,230,230,0.5)] backdrop-blur-sm rounded-4xl w-4/5 h-7/10 scale-0 transition-all duration-300 ${isClicked ? 'scale-100 opacity-100' : ''} `}>
                ...
                <div className='about-link mx-auto py-3 text-black group cursor-pointer relative'>
                    ...
                    <div className='about-links w-56 bg-[rgba(230,230,230,.7)] backdrop-blur-lg rounded-2xl flex flex-col py-2 px-3 shadow absolute left-1/2 top-12 -translate-x-1/2 scale-0 group-hover:scale-100 transition-all duration-300'>
                        <a href="#about" className='text-black my-1 hover:translate-x-4 transition-transform duration-300' onClick={handleBar}>About GGLife</a><hr className='opacity-25' />
                        <a href="#showcase" className='text-black my-1 hover:translate-x-4 transition-transform duration-300' onClick={handleBar}>Why GGLife ?</a>
                    </div>
                </div>
                <a className='py-3 text-black mx-auto' onClick={handleBar} href="#how">How it works</a>
                <a className='py-3 text-black mx-auto' onClick={handleBar} href="#contact">Contact us</a>
                <a href="/signup" className='py-3 text-black' onClick={handleBar}>Get Started</a>
                <GGlink title='Login' takeTo={'/Login'} className='text-white mx-auto px-6' onClick={handleBar} />
            </div>
        </>
    )
}
